...
# REPEAT FOR BRES 2024 LOCAL AUTHORITY LEVEL (and do extra IndStrat checks)----

# Extra checks:
# Am I matching against IndStrat sectors correctly using the fuzzyjoin thing? Issue: some frontier and non-frontier sectors are nested
# Relatedly: check that job sums for each sector are correct, and so job %s. I think there might be issues.


#Excluded codes: Frontier areas without clear SIC mappings (e.g., Clean Tech, Hydrogen, Heat Pumps, Nuclear) not included
# That's an issue from the IndStrat codes itself, not something else
indsic <- read_csv('data/industrialstrategy2025sectordefs.csv')
# indsic <- read_csv('data/industrialstrategy2025sectordefs_test.csv')
...
bres_annotated <- fuzzyjoin::regex_left_join(
  bres,
  # strategy_df %>% distinct(sic_code, .keep_all = T),#Some like 20 are doubled up in e.g. foundationals, keep only one
  strategy_df,#Actually, need all since doublings apply to different indstrat sectors
  by = c("SIC_5DIGIT_CODE" = "regex")
)

# Have a look at the result e.g. SIC 60 should apply to two different instrat cats...
# Yep so that got two 5 digits, and left_join doubled them up as they both match different indstrat categories
bres_annotated %>% filter(sic_code == 60, DATE == 2024, GEOGRAPHY_NAME == 'Sheffield') %>% View
# These two pairs are both present, though nested. Did they both get there OK?
# Yep all present and correct and so can be summed by those specific indstrat cats
bres_annotated %>% filter(sic_code %in% c(58,5821,62011,6201), DATE == 2024, GEOGRAPHY_NAME == 'Sheffield') %>% View

#Then just summarise
#Don't need to keep indstrat sectors we have no match for
# We may as well keep this just to indstrat categories for now
# And then work out job totals and percents later
# As those need doing at different SIC digit levels, it's fiddly

# To state again what we need here:
# Separate job sums for matching SIC codes that are in separate IndStrat cats
# E.g. 60 is in *both* creative and digital/tech
# AND separate job sums for nested SIC codes
# E.g. 58 and 5821, both in creative industries (and other indstrat cats too)


# Let's just change one or two column names to make this easier to comprehend!
bres_summary <- bres_annotated %>%
  rename(indstrat_category = sector_name) %>% 
  filter(!is.na(sic_code)) %>% 
  group_by(DATE,GEOGRAPHY_NAME,SIC_name,indstrat_category) %>%
  summarise(
    JOBCOUNT = sum(JOBCOUNT, na.rm = TRUE), .groups = "drop",
    sic_codefromindstrat = max(sic_code),
    # topsector_indstrat = max(sector_name),
...
# Let's look again at the same subsets as above to check summarising did the right thing
# Looking all present and correct
bres_summary %>% filter(sic_codefromindstrat == 60, DATE == 2024, GEOGRAPHY_NAME == 'Sheffield') %>% View
bres_summary %>% filter(sic_codefromindstrat %in% c(58,5821,62011,6201), DATE == 2024, GEOGRAPHY_NAME == 'Sheffield') %>% View
...
bres_summary <- bres_summary %>% 
  arrange(DATE) %>% 
  group_by(GEOGRAPHY_NAME,indstrat_category,sic_namefrom_indstrat) %>%
  mutate(
    jobcount_movingav = rollapply(JOBCOUNT,smoothband,mean,align='center',fill=NA)
  ) %>% 
  ungroup() %>% 
...
# Look again! Change date to 2023, which is av for 2022-24
bres_summary %>% filter(sic_codefromindstrat == 60, DATE == 2023, GEOGRAPHY_NAME == 'Sheffield') %>% View
bres_summary %>% filter(sic_codefromindstrat %in% c(58,5821,62011,6201), DATE == 2023, GEOGRAPHY_NAME == 'Sheffield') %>% View



#MAKE SHORTER NAMES THAT COMBINE WITH THE INDSTRAT TOP CAT
#Top level cats first, that we will combine with the shortened sector name
unique(bres_summary$indstrat_category)

bres_summary <- bres_summary %>% 
  mutate(
    toplevelindstrat = case_when(
      qg('manufactur', indstrat_category) ~ "ADV MANUF",
      qg('business', indstrat_category) ~ "PBS",
      qg('financ', indstrat_category) ~ "FIN",
      qg('creative', indstrat_category) ~ "CREATIVE",
      qg('foundation', indstrat_category) ~ "FOUNDATION",
      qg('digital', indstrat_category) ~ "DIGITAL",
      qg('life sci', indstrat_category) ~ "LIFESCI",
      qg('defence', indstrat_category) ~ "DEFENCE"
    )
  )

unique(bres_summary$toplevelindstrat)
...
bres_summary <- bres_summary %>% 
  mutate(
    sic_namefrom_indstrat_combo = paste0(toplevelindstrat,": ",sic_namefrom_indstrat_short, " (",sic_codefromindstrat,")")
  )
...
# Let's look again at our subsets...
islq %>% filter(sic_codefromindstrat == 60, DATE == 2023, GEOGRAPHY_NAME == 'Sheffield') %>% View
islq %>% filter(sic_codefromindstrat %in% c(58,5821,62011,6201), DATE == 2023, GEOGRAPHY_NAME == 'Sheffield') %>% View
...
#Save all the bits! So we can repeat for different places easily
saveRDS(islq,'local/islq2024.rds')
saveRDS(yeartoplot,'local/is_bres_yeartoplot2024.rds')
...
#save a copy of that to save processing time in quarto
saveRDS(yeartoplot.lqfiltered,'local/data/bres_indstrat_LQs_LAlevel2024.rds')
...
# BRES 2024 LOCAL AUTHORITY LEVEL IndStat attempt 2, doing LQs properly----

# Previously: putting the indstrat job sums into LQ calcs = regional and national proportions
# ONLY for indstrat sectors
# And now I'm doubling up some of those sectors, that makes even less sense, is totaly wrong

# Various ways to address
# First try: run LQ calcs for each SIC section level from BRES, so they're correct
# Then merge those in to the indstrat categories
