...
3.  An example of **grabbing code from github** to run in RStudio online ([jump here](setupRonline_n_getstarted_revealjs.html#/grabbing-some-code-from-github-a-gva-x-jobs-blocks-example) to skip R intro). The script you'll grab is self-contained: all code and packages are included, and the #codecomments run you through its use. (In this example, we'll make a GVA \* jobs 'block' plot. See comments for details...)
