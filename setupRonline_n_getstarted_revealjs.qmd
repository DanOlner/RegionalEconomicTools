...
-   Here's an example of an R script that will run **entirely self-contained** in posit.cloud (if you've used the tidyverse template). It loads some GVA and jobs data and a geography lookup. On first run, it'll install the [ggrepel](https://ggrepel.slowkow.com) package for some better labels.
...
-   The code will plot GVA vs GVA-per-fulltime-job in sector blocks, stacked in order of productivity (gva per full time job). We get to see some interesting things.
...
1. The **middle 'copy raw file' button**: click this and github will stick the whole script onto the clipboard, ready for pasting into an R script - just as we did above with the single line of code.
...
* Once that code is into an R script, we can **run it chunk by chunk from top to bottom**, as described in [this slide earlier](setupRonline_n_getstarted_revealjs.html#/5/6). 
* Comments in the script talk through what each chunk is doing.
