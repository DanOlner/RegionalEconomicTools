...
1.  A [Yorkshire and Humber sector catalogue](https://danolner.github.io/RegionalEconomicTools/quarto_docs/Yorkshire_n_Humber_sector_catalogue_November2025.html){target="_blank"} that includes detailed analysis of sector strengths across the region. **This sector catalogue document contains the most granular detail**, has a full guide and explanation on how to use it and a 3000 word write-up of stand-outs from the data analysis, broken down by sector.
...
## Industrial strategy: location quotients for each IS-8, for each local authority in Y&H

Calculated separately to make sure LQs are accurate per IS-8 grouping.
