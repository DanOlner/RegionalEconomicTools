---
title: "YNY productivity bits Oct 2025"
sidebar: false
author:
  - name: "Dan Olner"
format: 
  html:
    output-file: "YNY_productivity_bitsandbobs_Oct2025"
    fig-width: 8
    fig-height: 8
execute:
  echo: false
...
#Mark 4 YNY LAs and plot
slopes.hours = slopes.hours %>% 
  ungroup() %>% 
  # mutate(
  #   SY_LAs = ifelse(qg('rotherh|doncaste|sheffie|barnsley', Region_name),'YNY LA','Other'),
  #   corecities = ifelse(Region_name %in% corecities,'core city','Other'),
  #   london = ifelse(qg('london',Region_name), 'London','Other')
  # )
  mutate(
    itl3label = case_when(
      # qg('york', Region_name) ~ 'YNY LA',#will overwrite sheffield as core city
      Region_name %in% c('York','North Yorkshire') ~ 'YNY LA',#will overwrite sheffield as core city
      Region_name %in% corecities ~ 'Core city',
      Region_name %in% itl3.lon$ITL325NM ~ 'London',
      .default = 'Other'
      ),
    itl3label = factor(itl3label, ordered = T, levels = c('Other','London','Core city','YNY LA'))
  )
...
# check names
# getdistinct('york',slopes.gva$Region_name)
...
slopes.gva = slopes.gva %>% 
  ...
  mutate(
    itl3label = case_when(
      # qg('york', Region_name) ~ 'YNY LA',#will overwrite sheffield as core city
      # qg('rotherh|doncaste|sheffie|barnsley', Region_name) ~ 'YNY LA',#will overwrite sheffield as core city
      Region_name %in% c('York','North Yorkshire') ~ 'YNY LA',#will overwrite sheffield as core city
      Region_name %in% corecities ~ 'Core city',
      Region_name %in% itl3.lon$ITL325NM ~ 'London',
      .default = 'Other'
...
p = addtogva_hours_plot(p, both %>% filter(itl3label == 'YNY LA'), T, 7, RColorBrewer::brewer.pal(3, "Set1")[3], 1)
