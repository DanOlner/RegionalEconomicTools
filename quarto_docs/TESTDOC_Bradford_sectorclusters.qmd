...
# for(i in 1:3){
#   
#   # Output heading as raw markdown
#   cat(knitr::asis_output(paste0("## Heading ", i, "\n\n")))
#   
#   # Output some text
#   cat(i * 10, "\n\n")
#   
#   # Output plot
#   plot(1:100, main = paste("Plot", i))  # This will display properly
# }
...
for(i in 1:3){
  cat("## Heading ", i, "\n\n")
  cat("Value: ", i * 10, "\n\n")
  # cat(paste0("plot(1:100, main = 'Plot ", i, "')\n\n"))
  cat(paste0("```{r plot-", i, ", echo=FALSE}\n",
             "plot(1:100, main = 'Plot ", i, "')\n",
             "```\n\n"))
}
...
titles <- paste("## Heading", 1:3)
values <- 1:3 * 10
plots <- map(1:3, ~ ggplot(data.frame(x = 1:100), aes(x, y = 1)) + 
                geom_line() + 
                ggtitle(paste("Plot", .x)))

# Store for use in later chunk
plot_data <- tibble::tibble(titles, values, plots)
...
walk2(plot_data$titles, plot_data$plots, function(title, plot) {
  cat(title, "\n\n")
  print(plot)
  cat("\n\n")
})
...
Testing labelling with figures. Does @fig-bresindstrat work or does it need the label? Nope, just name in header.


```{r eval=F, fig-bresindstrat}
#| fig-width: 9
