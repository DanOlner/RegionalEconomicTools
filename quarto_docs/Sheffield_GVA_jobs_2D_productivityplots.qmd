---
title: "Sheffield: GVA vs jobs 2D percent change plots"
author:
  - name: "Dan Olner"
format: 
  html:
    output-file: "Sheffield_2D_gvajobs_percentchangeplots_July2025"
    fig-width: 9
    fig-height: 9
execute:
  echo: false
...
These plots show **percent change in chained volume GVA and job count** on different axes, with **Sheffield's change overlaid in blue** and job count plus percent of regional economy included in title (for the most recent year). It uses a 3 year moving average to smooth out jumps.

Only places where a sector makes up 1.5% or more of the economy are included - except for Sheffield, which is included in all plots.
...
**NOTE: All sectors are included even if they're very small proportions of Sheffield's economy - so percent changes could appear large for very tiny sectors. Look at the job count in the title to check. Including them all for completeness.**
...
for(sector in unique(bres.gva.2digit.2023$SIC07_description_shortened)[order(unique(bres.gva.2digit.2023$SIC07_description_shortened))]){
  ...
  #Always plot Sheffield! Add in if not there
  if(!"Sheffield" %in% placestokeep) placestokeep = c(placestokeep,"Sheffield")
  ...
  p <- twod_percentplot(
    ...
    category_var_value_to_highlight = 'Sheffield',
    start_time = 2016,
    end_time = 2022
  )
  
  bradjobs <- gva.jobs.ITL3.for2dplot %>% filter(!is.na(jobcount_movingav), Region_name == "Sheffield", SIC07_description_shortened == sector, DATE == 2022) %>% select(jobcount_movingav) %>% pull 
  bradpercentjobs <- gva.jobs.ITL3.for2dplot %>% filter(!is.na(jobcount_movingav), Region_name == "Sheffield", SIC07_description_shortened == sector, DATE == 2022) %>% select(JOBS_sector_regional_percent_movingav) %>% pull %>% round(2)
