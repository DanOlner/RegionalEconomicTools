---
title: "HEYCA sector economy analysis plots"
author:
  - name: "Dan Olner"
format: 
  html:
    output-file: "HEYCA_sectorclusters_July2025"
    fig-width: 8
    fig-height: 8
execute:
  echo: false
...
places = getdistinct('upon hull|east riding',df$Region_name)
...
slopes.hours = slopes.hours %>% 
  ...
  mutate(
    itl3label = case_when(
      ...
      qg('upon hull|east riding', Region_name) ~ 'HEY LA',#will overwrite sheffield as core city
      Region_name %in% corecities ~ 'Core city',
      Region_name %in% itl3.lon$ITL325NM ~ 'London',
      .default = 'Other'
      ),
    itl3label = factor(itl3label, ordered = T, levels = c('Other','London','Core city','HEY LA'))
  )
...
slopes.gva = slopes.gva %>% 
  ...
  mutate(
    itl3label = case_when(
      # qg('york', Region_name) ~ 'HEY LA',#will overwrite sheffield as core city
      qg('upon hull|east riding', Region_name) ~ 'HEY LA',#will overwrite sheffield as core city
      Region_name %in% corecities ~ 'Core city',
      Region_name %in% itl3.lon$ITL325NM ~ 'London',
      .default = 'Other'
...
p = addtogva_hours_plot(p, both %>% filter(itl3label == 'HEY LA'), T, 7, RColorBrewer::brewer.pal(3, "Set1")[3], 1)
...
slopes.hours = slopes.hours %>% 
  ungroup() %>% 
  # mutate(
  #   SY_LAs = ifelse(qg('rotherh|doncaste|sheffie|barnsley', Region_name),'HEY LA','Other'),
  #   corecities = ifelse(Region_name %in% corecities,'core city','Other'),
  #   london = ifelse(qg('london',Region_name), 'London','Other')
  # )
  mutate(
...
slopes.gva = slopes.gva %>% 
  ...
  mutate(
    itl3label = case_when(
      # qg('york', Region_name) ~ 'HEY LA',#will overwrite sheffield as core city
      qg('', Region_name) ~ 'HEY LA',#will overwrite sheffield as core city
      Region_name %in% corecities ~ 'Core city',
      Region_name %in% itl3.lon$ITL325NM ~ 'London',
      .default = 'Other'
    ),
...
p <- p + 
  geom_point(
    position = position_nudge(y = -0.2),
    data = empchange.summary %>% filter(qg('upon hull|east riding',localauthority_name)),
...
firm.change <- ch %>% 
  filter(qg('upon hull|east riding',localauthority_name), Employees_thisyear >= 10 & Employees_lastyear >= 10) %>% #10+ in both time periods
  mutate(
